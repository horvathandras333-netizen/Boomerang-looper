...
class BoomerangApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        tk.Label(self, text="Forward → Reverse, crossfaded and cut to exact length.",
                 font=("Segoe UI", 9), bg=self.BG, fg=self.MUTED).pack(anchor="w", padx=24, pady=(4, 20))

        # file picker
        drop_frame = tk.Frame(self, bg=self.PANEL, highlightbackground=self.BORDER,
                              highlightthickness=1, cursor="hand2")
        drop_frame.pack(fill="x", padx=24, pady=(0, 16))
        drop_frame.bind("<Button-1>", lambda e: self._browse())

        self.drop_icon = tk.Label(drop_frame, text="🎬", font=("Segoe UI", 30),
        ...
        settings.pack(fill="x", padx=24, pady=(0, 20))
        ...
        tk.Label(cf_row, text="sec", font=("Segoe UI", 10), bg=self.BG, fg=self.MUTED).pack(side="left")
        ...
        self.run_btn = tk.Button(
            ...
            relief="flat", padx=0, pady=12, cursor="hand2",
            command=self._on_run_button
        )
        self.run_btn.pack(fill="x", padx=24, pady=(0, 24))
    ...
    # ── thread-safe UI helpers (safe to call from the worker thread) ──
    ...
    def _start(self):
        ...
        t = threading.Thread(target=self._process,
                             args=(src, out_path, target, cf), daemon=True)
        t.start()
    ...
    def _run_ffmpeg(self, args: list, step_name: str,
        ...
        # stderr goes to a real temp file (not a pipe) so we can drain stdout
        # for progress without risking a deadlock on a full stderr buffer.
    ...
    def _process(self, src: str, out: str, target: float, cf: float):
        ...
        try:
            ...
            have_audio = has_audio_stream(src)
            self._set_progress(5)

            # each cycle = forward + reverse, minus one crossfade overlap at the join
            cycle_dur = (clip_dur * 2) - cf if cf > 0 else clip_dur * 2
            if cycle_dur <= 0:
                raise ValueError("Crossfade is longer than the clip — reduce it.")

            # ceiling, plus one extra cycle of safety margin since crossfades
            # between cycles (not just within a cycle) also eat into runtime —
            # harmless because the output is hard-trimmed to `target` anyway.
            ...
            # Compatibility flags for editors like PowerDirector
            compat = [
                "-pix_fmt", "yuv420p",
                "-profile:v", "high",
                "-level", "4.1",
            ...
            # Encode the forward and reverse segments ONCE and reuse them for
            # every loop — the concat/xfade steps just reference these files
            # multiple times, instead of re-encoding the reverse pass per loop.
            fwd = os.path.join(tmpdir, "fwd.mp4")
            rev = os.path.join(tmpdir, "rev.mp4")

            self._set_status("Copying forward segment…")
            ...
            rev_cmd += ["-af", "areverse"] if have_audio else ["-an"]
            ...
            join_estimated_dur = cycle_dur * loops_needed + clip_dur  # rough, for progress only

            self._set_status(f"Joining {loops_needed} loop(s)…")

            if cf > 0:
                joined = self._xfade_concat(
                    ffmpeg, tmpdir, segments, cf, enc_args, have_audio,
                    seg_dur=clip_dur, progress_duration=join_estimated_dur,
                ...
                concat_cmd += ["-c:a", "aac", "-b:a", "192k"] if have_audio else ["-an"]
                concat_cmd += [joined]
                self._run_ffmpeg(concat_cmd, "joining segments",
                                  duration=join_estimated_dur, start_pct=50, end_pct=90)

            self._set_status(f"Trimming to {target}s…")
            trim_cmd = [ffmpeg, "-y", "-i", joined, "-t", str(target)] + enc_args()
            trim_cmd += ["-c:a", "aac", "-b:a", "192k"] if have_audio else ["-an"]
            trim_cmd += [out]
            self._run_ffmpeg(trim_cmd, "trimming to the target duration",
    ...
    def _xfade_concat(self, ffmpeg, tmpdir, segments, cf, enc_args_fn, have_audio,
                       seg_dur, progress_duration, start_pct, end_pct):
        """
        Chain N segments together with FFmpeg's xfade/acrossfade filters.
        `seg_dur` is the known duration of each individual segment (they all
        come from the same source clip, so this avoids re-probing each one).
        ...
        cumulative = seg_dur  # after first segment

        for i in range(1, n):
            offset = max(0.01, cumulative - cf)
            v_out = f"[vx{i}]"
            filter_lines.append(
                f"{v_cur}{vparts[i]}xfade=transition=fade:duration={cf}:offset={offset}{v_out}"
            )
            v_cur = v_out
            if have_audio:
                a_out = f"[ax{i}]"
                ...
                a_cur = a_out
            cumulative += seg_dur - cf  # each subsequent segment adds dur minus overlap
